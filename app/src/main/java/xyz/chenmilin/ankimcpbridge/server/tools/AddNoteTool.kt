...
class AddNoteTool(private val ankiRepository: AnkiRepository) : McpTool {
    ...
    override val definition = McpToolDef(
        name = "add_note",
        description = "按指定笔记类型（noteTypeId）向目标牌组写入一张笔记，支持任意字段（不限于 Basic 的 Front/Back）。字段名必须与 list_note_types / get_note_type 返回的字段名一致；未提供的字段写入空字符串，输入中出现未知字段会报错。Cloze 类型不会强制 Front/Back 规则。写入后会回读验证数据已持久化，并通知 AnkiDroid 刷新（本地刷新，非 AnkiWeb 云同步）。",
        inputSchema = JsonObject(
            mapOf(
                ...
                "properties" to JsonObject(
                    mapOf(
                        "deck" to JsonObject(
                            mapOf("type" to JsonPrimitive("string"), "description" to JsonPrimitive("目标牌组名称"))
                        ),
